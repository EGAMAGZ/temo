...
class MockElement {
  ...
  addEventListener(type: string, listener: Function, options?: { signal?: AbortSignal }) {
    ...
    if (options?.signal) {
      ...
      (options.signal as any)._abortListeners = (options.signal as any)._abortListeners || [];
  ...
  removeEventListener(type: string, listener: Function) {
    if (this._listeners[type]) {
      this._listeners[type] = this._listeners[type].filter(entry => entry.fn !== listener);
    }
  }
...
class MediaQueryListMock {
  ...
  addEventListener(type: string, listener: (e: MediaQueryListEvent) => void, options?: any) {
...
describe("Temo", () => {
  ...
  it("should initialize with default theme when no storage exists", () => {
    const temo = Temo.init({ defaultTheme: "light", storageKey: "theme", autoDetect: false });
  ...
  it("should use custom storage key", () => {
    ...
    const temo = Temo.init({ defaultTheme: "dark", storageKey: customKey, autoDetect: false });
  ...
  it("should be singleton - return same instance", () => {
    const temo1 = Temo.init({ defaultTheme: "light", storageKey: "theme", autoDetect: false });
    const temo2 = Temo.init({ defaultTheme: "dark", storageKey: "other", autoDetect: true });
  ...
  it("should destroy and clean up event listeners", () => {
    ...
    mockButton.click();
    assertEquals(document.documentElement.getAttribute("data-theme"), initialTheme);
  });

  it("should auto-detect dark theme preference and handle media query changes", () => {
    prefersDark = true; // Set mock to prefer dark theme
    const temo = Temo.init({ autoDetect: true, storageKey: "theme", defaultTheme: "light" });
    assertEquals(globalThis.localStorage.getItem("theme"), "dark");
    assertEquals(document.documentElement.getAttribute("data-theme"), "dark");
